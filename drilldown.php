...
$tot = mysqli_query($conn, "SELECT sum(LineTotal) as tot from fact_sales");
...
$sql = "SELECT concat('name:',st.Name) as name, concat('y:', sum(fs.LineTotal)*100/" . $tot_amount[0] . ") as y, concat('drilldown:', st.Name) as drilldown
            FROM salesterritory st
            JOIN fact_sales fs ON (st.TerritoryID = fs.TerritoryID)
            GROUP BY name
            ORDER BY y DESC";
//echo $sql;
...
$sql = "SELECT st.name kategori, sum(fs.lineTotal) as tot_kat
            FROM fact_sales fs
            JOIN salesterritory st ON (st.TerritoryID = fs.TerritoryID)
            GROUP BY kategori";
...
$sql = "SELECT st.name kategori, 
            t.bulan as bulan, 
            sum(fs.linetotal) as pendapatan_kat
            FROM salesterritory st
            JOIN fact_sales fs ON (st.TerritoryID = fs.TerritoryID)
            JOIN time t ON (t.time_id = fs.TimeID)
